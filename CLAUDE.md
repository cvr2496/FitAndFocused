# FitAndFocused - Claude AI Assistant Guide
...
- **AnthropicService** (`app/Services/AnthropicService.php`): Handles Claude API calls
  ...
  - IMPORTANT: Always check `mockMode` property before making real API calls
...
- **Workout**: Core model with PHPDoc annotations for PHPStan
  - Must include `@property` declarations for all attributes
...
## Workflow Tips
1. **Explore first**: Use `view_file_outline` to understand structure before editing
2. **Plan changes**: Create implementation plans for multi-file changes
3. **Commit incrementally**: Break work into database, services, controllers, frontend, cleanup
